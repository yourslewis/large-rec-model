...
def _compute_pred_and_logits(
    ...
    # interaction = encoded_user_embeddings
    # # add a small item_embeddings to keep the gradient flowing
    # interaction = interaction + 0.0 * item_embeddings.mean(dim=-1, keepdim=True)
    # mt_logits = prediction_module(interaction).transpose(0, 1)
    mt_logits = prediction_module(encoded_user_embeddings * item_embeddings).transpose(
        0, 1
    ) # "item_embeddings": *, +, remove
