...
from common import HammerModule
...
class ActionEncoder(HammerModule):
    def __init__(
        ...
        # )
        self._action_embedding_table: torch.nn.Parameter = torch.nn.Parameter(
            torch.zeros((self._num_action_types, action_embedding_dim)),
            requires_grad=False
        )
        self._target_action_embedding_table: torch.nn.Parameter = torch.nn.Parameter(
            torch.zeros((1, self._num_action_types * action_embedding_dim)),
            requires_grad=False
        )
    ...
    def forward(
        ...
        exploded_actions = (
            torch.bitwise_and(
                seq_actions.unsqueeze(-1), self._combined_action_weights.unsqueeze(0) # _combined_action_weights is just the action_weights when _watchtime_to_action_thresholds_and_weights is empty (in msan)
            )
            > 0
        )
        action_embeddings = (
        ...
        total_targets = int(target_offsets[-1].item())
