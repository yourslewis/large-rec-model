...
class DataProcessor:
    ...
    def output_format_csv(self) -> str:
        return f"dlrm_v3/data/{self._prefix}/sasrec_format.csv"
...
class MovielensDataProcessor(DataProcessor):
    ...
    def download(self) -> None:
        ...
        if self._saved_name[-4:] == ".zip":
            ZipFile(self._saved_name, "r").extractall(path="dlrm_v3/data/")
        else:
            with tarfile.open(self._saved_name, "r:*") as tar_ref:
                tar_ref.extractall("dlrm_v3/data/")

    def processed_item_csv(self) -> str:
        return f"dlrm_v3/data/processed/{self._prefix}/movies.csv"

    def sasrec_format_csv_by_user_train(self) -> str:
        return f"dlrm_v3/data/{self._prefix}/sasrec_format_by_user_train.csv"

    def sasrec_format_csv_by_user_test(self) -> str:
        return f"dlrm_v3/data/{self._prefix}/sasrec_format_by_user_test.csv"
    ...
    def preprocess_rating(self) -> int:
        ...
        if self._prefix == "ml-1m":
            users = pd.read_csv(
                f"dlrm_v3/data/{self._prefix}/users.dat",
                sep="::",
                names=["user_id", "sex", "age_group", "occupation", "zip_code"],
            )
            ratings = pd.read_csv(
                f"dlrm_v3/data/{self._prefix}/ratings.dat",
                sep="::",
                names=["user_id", "movie_id", "rating", "unix_timestamp"],
            )
            movies = pd.read_csv(
                f"dlrm_v3/data/{self._prefix}/movies.dat",
                sep="::",
            ...
            ratings = pd.read_csv(
                f"dlrm_v3/data/{self._prefix}/ratings.csv",
                sep=",",
            )
            ratings.rename(
                columns={
            ...
            movies = pd.read_csv(
                f"dlrm_v3/data/{self._prefix}/movies.csv",
                sep=",",
                encoding="iso-8859-1",
            )
            ...
            movie_ids = []
            for i in range(16):
                train_file = f"dlrm_v3/data/{self._prefix}/trainx16x32_{i}.npz"
        ...
        # Save primary csv's
        if not os.path.exists(f"dlrm_v3/data/processed/{self._prefix}"):
            os.makedirs(f"dlrm_v3/data/processed/{self._prefix}")
        if users is not None:
            users.to_csv(f"dlrm_v3/data/processed/{self._prefix}/users.csv", index=False)
        if movies is not None:
            movies.to_csv(f"dlrm_v3/data/processed/{self._prefix}/movies.csv", index=False)
        ratings.to_csv(f"dlrm_v3/data/processed/{self._prefix}/ratings.csv", index=False)
...
class AmazonDataProcessor(DataProcessor):
    ...
    def preprocess_rating(self) -> int:
        ...
        if not os.path.exists(f"dlrm_v3/data/{self._prefix}"):
            os.makedirs(f"dlrm_v3/data/{self._prefix}")
...
def get_common_preprocessors() -> (
    ...
    amzn_books_dp = AmazonDataProcessor(  # pyre-ignore [45]
        ...
        "dlrm_v3/data/ratings_Books.csv",
        prefix="amzn_books",
