...
def eval_metrics_v2_from_tensors(
    ...
    model: SimilarityModule,                        # type is wrong
    seq_features: SequentialFeatures,               # past_ids, past_ratings, (past_timestamps + 1)
    target_ids: torch.Tensor,                       # [B, 1]
    min_positive_rating: int = 4,                   # default to be 4
    target_ratings: Optional[torch.Tensor] = None,  # [B, 1]
    epoch: Optional[str] = None,
    filter_invalid_ids: bool = True,                # default to be true
    ...
    user_max_batch_size = user_max_batch_size or shared_input_embeddings.size(0)     # B
    num_batches = (                                                                  # 1
        shared_input_embeddings.size(0) + user_max_batch_size - 1
    ) // user_max_batch_size
    eval_top_k_ids_all = []                # top-k items ids
    eval_top_k_prs_all = []                # top-k items prediction scores (probabilities)
    ...
    eval_ranks = torch.where(eval_rank_indices == k, MAX_K + 1, eval_rank_indices + 1)  # shape [B]
