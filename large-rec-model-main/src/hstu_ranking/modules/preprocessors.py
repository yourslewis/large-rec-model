...
class ContextualPreprocessor(InputPreprocessor):
    def __init__(
        ...
        self._max_contextual_seq_len: int = sum(
            contextual_feature_to_max_length.values() # in msan, contextual_feature_to_max_length is empty
        ) # if empty, then _max_contextual_seq_len will be 0
        self._contextual_feature_to_min_uih_length: Dict[str, int] = (
            contextual_feature_to_min_uih_length
        )
        if self._max_contextual_seq_len > 0: # in msan, the condition is false, skip this
            std = 1.0 * sqrt(
    ...
    def forward(  # noqa C901
        ...
        if self._action_weights is not None:
            uih_lengths = seq_lengths - num_targets
            uih_offsets = torch.ops.fbgemm.asynchronous_complete_cumsum(uih_lengths)
            ...
            action_embeddings = self._action_encoder(
                ...
                seq_embeddings=seq_embeddings, # actually not used
