...
def train_fn(
    ...
    world_size: int,
    master_port: int,
    dataset_name: str = "ml-20m",     # set to be "ml-1m"
    max_sequence_length: int = 200,     # set to be 200
    positional_sampling_ratio: float = 1.0,
    local_batch_size: int = 128,        # set to be 128
    eval_batch_size: int = 128,
    eval_user_max_batch_size: Optional[int] = None,
    main_module: str = "SASRec",        # set to be "HSTU"
    main_module_bf16: bool = False,
    dropout_rate: float = 0.2,          # set to be 0.2
    user_embedding_norm: str = "l2_norm",   # set to be "l2_norm"
    sampling_strategy: str = "in-batch",        # set to be "local"
    loss_module: str = "SampledSoftmaxLoss",     # set to be "SampledSoftmaxLoss"
    loss_weights: Optional[Dict[str, float]] = {},
    num_negatives: int = 1,                     # set to be 128
    loss_activation_checkpoint: bool = False,
    item_l2_norm: bool = False,       # set to be True
    temperature: float = 0.05,        # set to be 0.05
    ...
    partial_eval_num_iters: int = 32,       # default to be 32
    embedding_module_type: str = "local",
    item_embedding_dim: int = 240,      # set to be 50
    interaction_module_type: str = "",      # set to be "DotProduct"
    gr_output_length: int = 10,
    l2_norm_eps: float = 1e-6,         # set to be 1e-6
    ...
    dataset = get_reco_dataset(
        dataset_name=dataset_name,            # set to be "ml-1m"
        max_sequence_length=max_sequence_length,           # set to be 200
        chronological=True,
        positional_sampling_ratio=positional_sampling_ratio,        # default to be 1.0
    )

    train_data_sampler, train_data_loader = create_data_loader(
        dataset.train_dataset,
        batch_size=local_batch_size,     # set to be 128
    ...
    eval_data_sampler, eval_data_loader = create_data_loader(
        ...
        batch_size=eval_batch_size,      # default to be 128
        world_size=world_size,
        rank=rank,
    ...
    interaction_module, interaction_module_debug_str = get_similarity_function(
        module_type=interaction_module_type,          # set to be "DotProduct"
        query_embedding_dim=item_embedding_dim,       # set to be 50
        item_embedding_dim=item_embedding_dim,        # set to be 50
    )

    assert (
        user_embedding_norm == "l2_norm" or user_embedding_norm == "layer_norm"      # set to be "l2_norm"
    ...
    input_preproc_module = LearnablePositionalEmbeddingInputFeaturesPreprocessor(
        max_sequence_len=dataset.max_sequence_length + gr_output_length + 1,                 # set to be 200 + 10(default) + 1 = 211
        embedding_dim=item_embedding_dim,                                                    # set to be 50
        dropout_rate=dropout_rate,                                                           # set to be 0.2
    )

    model = get_sequential_encoder(
        module_type=main_module,                                                       # set to be "HSTU"
        max_sequence_length=dataset.max_sequence_length,                               # set to be 200
        max_output_length=gr_output_length + 1,                                        # set to be 10 + 1 = 11
    ...
    # loss
    loss_debug_str = loss_module                                            # set to be "SampledSoftmaxLoss"
    if loss_module == "BCELoss":
        ...
        loss_debug_str = "ssl"
        if temperature != 1.0:                                             # set to be 0.05
            loss_debug_str += f"-t{temperature}"
        ar_loss = SampledSoftmaxLoss(
            num_to_sample=num_negatives,                                   # set to be 128
            softmax_temperature=temperature,
            model=model,
            activation_checkpoint=loss_activation_checkpoint,              # default to be False
        )
        loss_debug_str += (
    ...
    if sampling_strategy == "in-batch":
        negatives_sampler = InBatchNegativesSampler(
            l2_norm=item_l2_norm,                      # set to be True
            l2_norm_eps=l2_norm_eps,                   # set to be 1e-6
            dedup_embeddings=True,
        )
        sampling_debug_str = (
            f"in-batch{f'-l2-eps{l2_norm_eps}' if item_l2_norm else ''}-dedup"
        )
    elif sampling_strategy == "local":                                      # set to be "local"
    ...
    device = rank
    if main_module_bf16:                                 # default to be False
    ...
    opt = torch.optim.AdamW(
        model.parameters(),
        lr=learning_rate,              # set to be 1e-3
        betas=(0.9, 0.98),
        weight_decay=weight_decay,     # set to be 0
    )
    ...
    if full_eval_every_n > 1:                      # default to be 1
        model_desc += f"-fe{full_eval_every_n}"
    if positional_sampling_ratio is not None and positional_sampling_ratio < 1:          # default to be 1
    ...
    log_dir = os.environ.get('TENSORBOARD_LOG_DIR', '.') + "/" + f"exps/{model_desc}"
    if rank == 0:
    ...
    torch.autograd.set_detect_anomaly(True)       # disable this during normal training for better performance

    batch_id = 0
    epoch = 0
    for epoch in range(num_epochs):                  # set to be 101
        if train_data_sampler is not None:           # DistributedSampler
            train_data_sampler.set_epoch(epoch)
        if eval_data_sampler is not None:            # DistributedSampler
            eval_data_sampler.set_epoch(epoch)
        model.train()
        for row in iter(train_data_loader):
            seq_features, target_ids, target_ratings = movielens_seq_features_from_row(     # seq_features: past_ids, past_ratings, (past_timestamps + 1)
                row,
                device=device,
                max_output_length=gr_output_length + 1,      # default to be 10 + 1
            )

            if (batch_id % eval_interval) == 0:              # default to be 100, evaluate every 100 steps
                model.eval()
                ...
                eval_state = get_eval_state(
                    ...
                    top_k_module_fn=lambda item_embeddings, item_ids: get_top_k_module(
                        top_k_method=top_k_method,                           # set to be "MIPSBruteForceTopK"
                        model=model.module,
                        item_embeddings=item_embeddings,
                        item_ids=item_ids,
                    ),
                    device=device,
                    float_dtype=torch.bfloat16 if main_module_bf16 else None,       # default to be False, float_dtype=None here
                )
                eval_dict = eval_metrics_v2_from_tensors(
                    eval_state,
                    model.module,
                    seq_features,                    # past_ids, past_ratings, (past_timestamps + 1)
                    target_ids=target_ids,
                    target_ratings=target_ratings,
                    user_max_batch_size=eval_user_max_batch_size,                   # default to be None
                    dtype=torch.bfloat16 if main_module_bf16 else None,             # default to be False, dtype=None
                )
                add_to_summary_writer(
            ...
            seq_features.past_ids.scatter_(                       #  past_ids + 1
                dim=1,
            ...
            opt.zero_grad()
            input_embeddings = model.module.get_item_embeddings(seq_features.past_ids)         # past_ids here contain target_ids as well, shape: [B,]
            # logging.info(f"past_ids shape {seq_features.past_ids.shape}")                    # [128, 211]
            # logging.info(f"intput_embeddings shape {input_embeddings.shape}")                # [128, 211, 50]
            seq_embeddings = model(
                past_lengths=seq_features.past_lengths,
                past_ids=seq_features.past_ids,
                past_embeddings=input_embeddings,
                past_payloads=seq_features.past_payloads,                                      # past_ratings, (past_timestamps + 1)
            ) 
            # logging.info(f"seq_embeddings shape {seq_embeddings.shape}")                     # [128, 211, 50]

            supervision_ids = seq_features.past_ids

            if sampling_strategy == "in-batch":                        # set to be "local"
            ...
            loss, aux_losses = ar_loss(
                ...
                negatives_sampler=negatives_sampler,
                **seq_features.past_payloads,                         # past_ratings, (past_timestamps + 1)
            )  # [B, N]

            main_loss = loss.detach().clone()
            loss = get_weighted_loss(loss, aux_losses, weights=loss_weights or {})     # default to be {}
        ...
        model.eval()
        eval_state = get_eval_state(                           # build CandidateIndex and TopKModule
            model=model.module,
            all_item_ids=dataset.all_item_ids,
            negatives_sampler=negatives_sampler,
            top_k_module_fn=lambda item_embeddings, item_ids: get_top_k_module(
        ...
        for eval_iter, row in enumerate(iter(eval_data_loader)):
            ...
            if (eval_iter + 1 >= partial_eval_num_iters) and (not is_full_eval(epoch)):               # always False
                logging.info(
