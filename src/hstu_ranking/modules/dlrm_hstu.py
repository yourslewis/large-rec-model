...
from modules.hstu_transducer import HSTUTransducer
from modules.multitask_module import (
    DefaultMultitaskModule,
    MultitaskTaskType,
    TaskConfig,
)
from modules.positional_encoder import HSTUPositionalEncoder
from modules.postprocessors import (
    LayerNormPostprocessor,
    TimestampLayerNormPostprocessor,
)
from modules.preprocessors import ContextualPreprocessor
from modules.stu import STU, STULayer, STULayerConfig, STUStack
from ops.jagged_tensors import concat_2D_jagged
from ops.layer_norm import LayerNorm, SwishLayerNorm
...
class DlrmHSTUConfig:
    max_seq_len: int = 256
    max_num_candidates: int = 1  # training
    max_num_candidates_inference: int = 1  # is_inference = True
...
class DlrmHSTU(HammerModule):
    def __init__(  # noqa C901
        ...
        # -------------- Embedding modules --------------
        if embedding_file_paths:  # If precomputed embedding file paths are provided
            # Use mixed mode
            self._use_precomputed_embeddings = True
            self._feature_to_table_mapping = {}  # Feature name to table name mapping
            remaining_tables = []
            
            # Build feature name to table name mapping
            for table_name, config in embedding_tables.items():
                for feature_name in config.feature_names:
                    self._feature_to_table_mapping[feature_name] = table_name
            
            # Create precomputed modules and register them as submodules
            precomputed_modules = torch.nn.ModuleDict()
            for table_name, config in embedding_tables.items():
                if table_name in embedding_file_paths:
                    precomputed_modules[table_name] = PrecomputedEmbeddingModule(
                        embedding_file_path=embedding_file_paths[table_name],
                        output_dim=config.embedding_dim,
                        preload=False,
                    )
                    logger.info(f"Using precomputed embeddings for table {table_name} (features: {config.feature_names})")
                else:
                    remaining_tables.append(config)
            
            # Register as submodule so it gets moved to correct device
            self._precomputed_modules = precomputed_modules
            
            # Create standard EmbeddingCollection for remaining features
            if remaining_tables:
                self._standard_embedding_collection = EmbeddingCollection(
                    tables=remaining_tables,
                    need_indices=False,
                    device=torch.device("meta"),
                )
            else:
                self._standard_embedding_collection = None
        else:
            # Use original approach
            self._use_precomputed_embeddings = False
            self._feature_to_table_mapping = {}
            self._standard_embedding_collection = EmbeddingCollection(
                tables=list(embedding_tables.values()),
                need_indices=False,
                device=torch.device("meta"),
            )
            self._precomputed_modules = torch.nn.ModuleDict()
        # -------------- Embedding modules end --------------
    ...
    def preprocess(
        ...
        if self._use_precomputed_embeddings:
            seq_embeddings_dict = {}
            
            # Process precomputed embeddings
            for feature_name in merged_sparse_features.keys():
                table_name = self._feature_to_table_mapping.get(feature_name)
                if table_name and table_name in self._precomputed_modules:
                    item_ids = merged_sparse_features[feature_name].values()
                    embeddings = self._precomputed_modules[table_name].get_item_embeddings(item_ids)
                    
                    # Reconstruct KeyedJaggedTensor format
                    seq_embeddings_dict[feature_name] = KeyedJaggedTensor(
                        keys=[feature_name],
                        values=embeddings.view(-1, embeddings.size(-1)),
                        lengths=merged_sparse_features[feature_name].lengths(),
                    )
            
            # Process standard embeddings
            if self._standard_embedding_collection is not None:
                # Extract features that need standard embeddings
                standard_feature_names = []
                for feature_name in merged_sparse_features.keys():
                    table_name = self._feature_to_table_mapping.get(feature_name)
                    if not table_name or table_name not in self._precomputed_modules:
                        standard_feature_names.append(feature_name)
                
                if standard_feature_names:
                    standard_features = KeyedJaggedTensor.from_lengths_sync(
                        keys=standard_feature_names,
                        values=torch.cat([
                            merged_sparse_features[k].values() 
                            for k in standard_feature_names
                        ]),
                        lengths=torch.cat([
                            merged_sparse_features[k].lengths() 
                            for k in standard_feature_names
                        ]),
                    )
                    
                    standard_embeddings = self._standard_embedding_collection(standard_features)
                    seq_embeddings_dict.update(standard_embeddings)
        else:
            # Use EmbeddingCollection
            seq_embeddings_dict = self._standard_embedding_collection(merged_sparse_features)
        # -------------- Precomputed Embeddings end --------------
        ...
        """
        seq_embeddings = {'event_id': SequenceEmbedding(...),
                          'item_event_id': SequenceEmbedding(...)}
        """
